...
We should now consider which components of the quaternion cause each rotation. Expressing $q$ as $\cos\theta + (x\vb{i} + y\vb{j} + z\vb{k})\sin\theta$, and $r$ as $b\vb{i} + c\vb{j} + d\vb{k}$:
\begin{equation}
	\begin{aligned}
		qr &= (\cos\theta + (x\vb{i} + y\vb{j} + z\vb{k})\sin\theta)(b\vb{i} + c\vb{j} + d\vb{k})
		   &= (b\vb{i} + c\vb{j} + d\vb{k})\cos\theta + (b\vb{i} + c\vb{j} + d\vb{k})(x\vb{i} + y\vb{j} + z\vb{k})\sin\theta
	\end{aligned}
\end{equation}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
At this point, it is important to consider the implications of the non-commutativity of quaternion rotation: while the product of two imaginary quaternion components differs based on the order of multiplication, this is not the case when a real number is multiplied by a quaternions component.
If we consider $qr$ as one point (after all it is equal ot a rotated point), we can then consider the product of $\bar{q}$ with this pure quaternion $qr$. A left multiplication of $\bar{q}$ would 
