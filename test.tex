...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}
        ...
        \draw[->, thick] (0,0) -- (1.5,1.809);
        \draw[dotted, thick] (1.5,0) coordinate (B) -- (1.5,1.809) coordinate (C);
        \node[anchor=north] at (0.75,0) {$\cos\theta$};
        \node[anchor=west] at (1.5,0.9045) {$\sin\theta$};
...
In this representation, the $\vb{i,j,k}$ space is thought of as one component, perpendicular to the real numbers. Considering the $\vb{i,j,k}$ space as defining the axis we are rotating around (in the same way that pure quaternions represent vectors - Section \ref{PureDef}), that axis will also be perpendicular to the real numbers. We can therefore think of a new diagram, Figure \ref{4DRotationsLike2DFig}, similar to Figure \ref{ComplexRotationFig}, but with the new one, the axis perpendicular to the real numbers will be the chosen axis to rotate around, instead of necessarily being $\vb{i}$. This axis, $A$, can be represented as a pure, unit quaternion, where:
\begin{equation}
...
Figure \ref{4DRotationsLike2DFig} shows diagramatically how we can use the same method for quaternion rotations as with 2-dimensional complex number rotations, by representing $q$ in the form:
\begin{equation}
    q = \cos\theta + A\sin\theta = \cos\theta + (x\vb{i} + y\vb{j} + z\vb{k})\sin\theta,
\end{equation}
where $q$ is the quaternion that a point (in pure quaternion form) is multiplied by for a rotation of $\alpha$ radians around the axis $A$. Do note that this does not rotate the point $\theta$ radians - the relation between $\theta$ and $\alpha$ here will be explained in Section \ref{QBarSection}.

There are two intuituve explanations for why cosine is used for the real component of this quaternion, and sine for the imaginary component.

Firstly, it can be explained graphically, in the same way as for complex number rotation: in short, multiplication by $A$ (analogous to $\vb{i}$ for complex numbers) makes the imaginary component of the rotation quaternion parallel to the opposite (to $\theta$) side of the theoretical right-angled triangle (Figure \ref{ComplexRotationFig}), and multiplying by $\sin\theta$ aligns its magnitude with it. See Section \ref{Complex2DRotation} for a much more in-depth explanation, which is applicable to quaternions as well, just by replacing $\vb{i}$ with the 3-dimensional axis of rotation, $A$. This method is less intuitive with quaternions, and better suited to complex numbers, due to the difficulty in accurately representing quaternions diagrammatically.

Perhaps the more intuitive method is to consider the sine and cosine functions themselves. For an angle of 0 (or any multiple of $2\pi$ - any full rotation), sine and cosine output 0 and 1 respectively. LINK THIS HERE TO THE DESIRED BEHAVIOUR WHEN "NOT" ROTATED (ROTATED BY 0) AND ANYTHING MORE RIGOROUS RELEVANT?

Finally, since $A$ is a (pure) unit quaternion (which implies $x^2 + y^2 + z^2 = 1$), and $\sin^2\theta + \cos^2\theta \equiv 1$, this will guarantee that $q$ will be a unit quaternion too, as desired:
\begin{equation}
    \begin{aligned}
...
combining rotations => why multiplication is not commutative
